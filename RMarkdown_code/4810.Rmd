...
### outlier detection:
```{r}
s = summary(fit4)$sigma

plot(fit4$fitted.values, fit4$residuals)
abline(h = 0, col = "red")
abline(h = c(-2, 2)*s, col = "red")
outliers <-  identify(fit4$fitted.values, fit4$residuals)
star2 <- star[-outliers, ]
```

```{r}
fit4.no.outliers <-  lm(formula = Calories ~ Total.Fat..g.  + Total.Carbohydrates..g. + 
    Cholesterol..mg. + Vitamin.C....DV.  + 
    Iron....DV. + Drink.Type +Drink.Type:Total.Carbohydrates..g., data = star2)
summary(fit4.no.outliers)

s2 <- summary(fit4.no.outliers)$sigma

plot(fit4.no.outliers$fitted.values, fit4.no.outliers$residuals)
abline(h = 0, col = "red")
abline(h = c(-2, 2)*s2, col = "red")
```
